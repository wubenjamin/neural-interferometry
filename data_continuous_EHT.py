...
class EHTIM_Dataset(Dataset):
    ...
    def __init__(self,  
            ...
            data_path = '../data/eht_grid_64FC_200im_Galaxy10_full.h5', 
            data_path_imgs = '../data/Galaxy10.h5', 
            data_path_cont = '../data/eht_cont_200im_Galaxy10_full.h5',
            img_res = 200,
            pre_normalize = False,
            ):
...
#        vis_sparse = np.stack((self.vis_re_sparse[:,idx], self.vis_im_sparse[:,idx]), axis=1)
        
        # normalize to -0.5,0.5
#        vis_real = vis.real.astype(np.float32)
#        vis_imag = vis.imag.astype(np.float32)
        ...
        if self.img_dataset:
            img_res_initial = int(torch.numel(self.img_dataset[idx][0])**(0.5))
            img = self.img_dataset[idx][0].reshape((img_res_initial,img_res_initial))
            if img_res_initial != self.img_res:
                #print('scaling input to match requested size:', img_res_initial, self.img_res)
        ...
        return self.uv_sparse.astype(np.float32), self.uv_dense.astype(np.float32), vis_sparse.astype(np.float32), vis_dense, img
    

    def __len__(self):
        return len(self.vis_re_sparse[0,:])
    
#    def get_metadata(self, idx, dset_name):
#        # dset_name: ['MNIST, 'Galaxy10']
#        from data_ehtim_cont import EHT_Continuous_Dataset
#        _, _, _, obs_meta = EHT_Continuous_Dataset(num_FC = self.fourier_res, 
#                                                   dset_name = dset_name)[idx]
#        return obs_meta
...
if __name__ == "__main__":
    ...
    data_path =f'../data/eht_grid_{fourier_resolution}FC_200im_{dset_name}_full.h5'

    #spectral_dataset = EHTIM_Dataset(data_path)
    #uv_sparse, uv_dense, vis_sparse, vis_dense = spectral_dataset[idx]
    
    im_data_path = '../data/Galaxy10.h5'
